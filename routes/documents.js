...
var DocumentsController = require('../controller/documents'); //Controlador con la logica para asociarla a un end-point
...
// definimos las rutas
api.post('/documents', DocumentsController.createDocument);
api.get('/documents',[md_auth.ensureAuth], DocumentsController.getDocuments); // en esta url necesitamos mandar el token en los headers
api.get('/documents/:id', DocumentsController.getDocument);
//api.post('/documents/insert-image/:id',[md_upload], DocumentsController.insertImage); // Antes de entrar en el controlador subira la imagen
api.put('/documents/:id', DocumentsController.updateDocument); //Ojo porque añadiste una / extra, inicialmente no existía la primera /.
api.delete('documents/:id', DocumentsController.deleteDocument)
